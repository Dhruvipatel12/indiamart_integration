...
def indiamart_api(startdate,enddate):
    # print("call..................................................")
    website_url = frappe.db.get_single_value("Indiamart Setting", "indiamart_url")
    glusr_crm_key = frappe.db.get_single_value("Indiamart Setting", "indiamart_key")
    # start_date, end_date = start_and_end()
    creating_url=f"{website_url}?glusr_crm_key={glusr_crm_key}&start_time={startdate}&end_time={enddate}"
    ...
    for x2 in x['RESPONSE']:
        ...
        for i in list:
            ...
            data= frappe.db.sql("select sender_id from `tabIndiaMart Enquiry` where sender_id=%s",i["UNIQUE_QUERY_ID"])
            dt=""
            for i1 in data:
                for j1 in i1:
                    dt=j1
            if dt != i["UNIQUE_QUERY_ID"]:
                a= frappe.get_doc(({"doctype" : "IndiaMart Enquiry",  "sender_id": i['UNIQUE_QUERY_ID'], "full_name": i['SENDER_NAME'], 	"mobile": i['SENDER_MOBILE'], "mobile_2": i['SENDER_MOBILE_ALT']
